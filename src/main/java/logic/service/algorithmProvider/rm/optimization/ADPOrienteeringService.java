...
import logic.algorithm.rm.optimization.learning.ADPWithSoftOrienteeringANN;
...
public class ADPOrienteeringService implements AlgorithmProviderService {
	...
	public SettingRequest getSettingRequest() {
		...
		if (algorithm != null && algorithm.equals("ADPWithSoftOrienteeringANN")) {
			String[] paras = ADPWithSoftOrienteeringANN.getParameterSetting();
			for (int i = 0; i < paras.length; i++) {
				request.addParameter(paras[i]);
			}
		}
	...
	public ValueFunctionApproximationModelSet startANNFunctionApproximation(int periodNumber)
		...
		Double samplePreferences = InputPreparator.getParameterValue(periodNumber, "samplePreferences");
		Double actualValue = InputPreparator.getParameterValue(periodNumber, "actualBasketValue");
		// Double initialiseProblemSpecific =
		// InputPreparator.getParameterValue(periodNumber,
		// "initialiseCoefficientsProblemSpecific");
		Double annealingTemp = InputPreparator.getParameterValue(periodNumber,
				"annealing_temperature_(Negative:no_annealing)");
		...
		Double momentumWeight = InputPreparator.getParameterValue(periodNumber, "momentum_weight");
		// Double targetInit = InputPreparator.getParameterValue(periodNumber,
		// "target_for_initialisation");
		Double explorationStrategy = InputPreparator.getParameterValue(periodNumber,
		...
		Double hTanActivation = InputPreparator.getParameterValue(periodNumber, "hTan_activation");
		...
		HashMap<Integer, ArrayList<OrderSet>> orderSetsForLearningH = InputPreparator
				.getLearningOrderSets(periodNumber);
		...
		HashMap<Integer, ArrayList<Routing>> routingsForBenchmarking = InputPreparator
				.getBenchmarkingFinalRoutings(periodNumber);
		HashMap<Integer, ArrayList<OrderSet>> orderSetsForBenchmarking = InputPreparator
				.getBenchmarkingOrderSets(periodNumber);
		...
		if (routingsForBenchmarking != null || orderSetsForBenchmarking != null) {
			EvaluationService.determineAverageOrderValuePerDeliveryArea(routingsForBenchmarking,
					orderSetsForBenchmarking, benchmarkValues, benchmarkingOrderRequestSets, deliveryAreaSet,
		...
		ADPWithOrienteeringANN algo = new ADPWithOrienteeringANN(region, demandSegmentWeighting,
				...
				daSegmentWeightingsUpper, daWeights, daSegmentWeightings, orderRequestSetsForLearning,
				objectiveSpecificValues, maximumRevenueValue, actualValue,
				orderRequestSetsForLearning.get(0).getBookingHorizon(), stepSize, annealingTemp, explorationStrategy,
				samplePreferences, momentumWeight, theftBased, theftBasedAdvanced, theftBasedTw,
				considerLeftOverPenalty, neighbors, arrivalProcessId, discountingFactor, discountingFactorProb,
				benchmarkValues, benchmarkingOrderRequestSets, additionalHiddenNodes, considerConstantAnn,
				considerNeighborDemand, oppOnlyFeasible, hTanActivation, expectedST);
	...
	public ValueFunctionApproximationModelSet startANNFunctionApproximationSoftOrienteering(int periodNumber)
			throws ParameterUnknownException {
		...
		Double includeDriveFromStartingPosition = InputPreparator.getParameterValue(periodNumber,
				"includeDriveFromStartingPosition");
		Double noRoutingCandidates = InputPreparator.getParameterValue(periodNumber, "no_routing_candidates");
		Double noInsertionCandidates = InputPreparator.getParameterValue(periodNumber, "no_insertion_candidates");
		Double distanceType = InputPreparator.getParameterValue(periodNumber, "distance_type");
		Double considerRemainingBudget = InputPreparator.getParameterValue(periodNumber, "consider_remaining_budget");
		Double considerRemainingTime = InputPreparator.getParameterValue(periodNumber, "consider_remaining_time");
		Double distanceInADP = InputPreparator.getParameterValue(periodNumber, "distance_adp");
		...
		HashMap<DeliveryArea, ArrayList<Double>> benchmarkValues = new HashMap<DeliveryArea, ArrayList<Double>>();
		ArrayList<OrderRequestSet> benchmarkingOrderRequestSets = new ArrayList<OrderRequestSet>();
		EvaluationService.determineAverageOrderValuePerDeliveryArea(routingsForBenchmarking, orderSetsForBenchmarking,
				benchmarkValues, benchmarkingOrderRequestSets, deliveryAreaSet, maximumRevenueValue,
				objectiveSpecificValues);

		ADPWithSoftOrienteeringANN algo = new ADPWithSoftOrienteeringANN(region, demandSegmentWeighting,
				vehicleAreaAssignmentSet, routingsForLearning, orderSetsForLearning, deliveryAreaSet, daWeightsUpper,
				daSegmentWeightingsUpper, daWeights, daSegmentWeightings, orderRequestSetsForLearning,
				objectiveSpecificValues, maximumRevenueValue, actualValue,
				orderRequestSetsForLearning.get(0).getBookingHorizon(), stepSize, annealingTemp, explorationStrategy,
				samplePreferences, momentumWeight, neighbors, arrivalProcessId, discountingFactor,
				discountingFactorProb, benchmarkValues, benchmarkingOrderRequestSets, additionalHiddenNodes,
				considerConstantAnn, hTanActivation, serviceTime, includeDriveFromStartingPosition, noRoutingCandidates,
				noInsertionCandidates, distanceType, considerRemainingBudget, considerRemainingTime, distanceInADP);
