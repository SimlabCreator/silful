...
public class S6l405_2S2c2cLowLowFlexibleInflexible7525 {

	String db = "SimLabS3V";
	String basicName = "S6l405_2S2c2cLowLowFlexibleInflexible7525_";
	...
	int noVehicles =3;
	int demandSegmentWeightingId = 46;
...
//	int firstOrderRequestSetIdForTOP=26604;
	...
	public S6l405_2S2c2cLowLowFlexibleInflexible7525() {
...
//				arrivalProcess, false);
	...
	public void runECTW() {
		...
		String name="#EcaTc_dType4PerTw";
		settings.put(name, new double[] { 0.0, 2.0, 1.0, 4.0, 1.0, 3.0, 0.0 });
		
		RunExperimentHelper helper = new RunExperimentHelper();
		HashMap<String, Double> paras = new HashMap<String, Double>();
		// Define settings
		helper.experimentName = basicName + "ADP" + name;
		...
		paras.put("consider_overall_remaining_capacity", settings.get(name)[0]);
		paras.put("consider_overall_accepted_insertion_costs", settings.get(name)[1]);
		paras.put("time_cap_interaction", settings.get(name)[2]);
		...
		paras.put("distance_type", settings.get(name)[3]);
		paras.put("distance_measure_per_tw", settings.get(name)[4]);
		paras.put("maximum_distance_measure_increase", settings.get(name)[5]);
		paras.put("switch_distance_off_point", settings.get(name)[6]);
