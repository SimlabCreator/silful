...
public class ExactDynamicProgrammingControls implements DynamicProgrammingAlgorithm {
	...
	public ExactDynamicProgrammingControls(int bookingHorizonLength, int arrivalProcessId,
	...
	private void solvePossibleSubProblems() throws ParameterUnknownException {
		...
		while (it.hasNext()) {
			...
			/// Define maximum values (maximum T, overall capacities) and
			...
			this.currentDimensions.put(currentArea, dimensions);

			// System.out.println("before it is
			// null?:"+MultipleDimensionalArrayProducer.read(this.treesPerDeliveryArea.get(currentArea),
			// dimensions));
			System.out.println("Start with delivery area "+currentArea.getId());
			Double value = this.getValue(dimensions, this.daSegmentWeightings.get(currentArea), currentArea);
						...
						if (this.objectiveSpecificValues.keySet().size() > 0) {
							...
							for (Entity obj : this.objectiveSpecificValues.keySet()) {
								...
								if (ow.getObjectiveType().getName().equals("social_impact_factor")) {
									...
									//if(segment.getId()==16) System.out.print(" weight social impact: "+ow.getValue());
...
//									//The second entry is the maximum value
									...
									weightedValue+=ow.getValue() * (maximumLocalVisibility-localVisibilityValues.get(currentDa).get(currentAlt.getAlternative()))/maximumLocalVisibility;
								}
							}

						}
						weightedValue += weightRevenue * meanRevenue;
						//	if(segment.getId()==16) System.out.print("weight revenue: "+weightRevenue);
						//	if(segment.getId()==16) System.out.println("demand segment: "+segment.getId()+"da: "+currentDa.getId()+" alt: "+currentAlt.getAlternativeId()+" weighted value: "+weightedValue);
