...
import logic.utility.comparator.ForecastedOrderRequestInsertionCostAscComparator;
...
/**
 * Computes insertion costs for every forecasted request Inserts requests into
 * route if feasible
 * 
 * @author C. K�hler
 *
 */
public class Old_InsertionConstructionHeuristicAdaption implements RoutingAlgorithm {

	boolean test = true; // TODO: DELETE
	...
	// double[][] timeDependentWeighting = { { 0, 0.95, 0.915, 0.87, 0.715,
	// 0.84, 1.075 },
	// { 0, 0.79, 0.795, 0.725, 0.615, 0.775, 1.1 }, { 0, 1.1, 0.775, 0.615,
	// 0.725, 0.795, 0.79 },
	// { 0, 0.79, 0.795, 0.725, 0.615, 0.775, 1.1 } };

	// adapted:in->in (cluster 6), in->out (cluster 5), out-> out(cluster 5),
	// out->in (flipped cluster 5, shift such that peak at 8-10)
	...
	private static String[] paras = new String[] { "Constant_service_time" , "Minimum_request_number"};

	public Old_InsertionConstructionHeuristicAdaption(ArrayList<ForecastedOrderRequest> requests,
			TimeWindowSet timeWindowSet, Depot depot, ArrayList<Vehicle> vehicles, ArrayList<NodeDistance> distances,
			ArrayList<Node> nodes, TravelTimeSet travelTimeSet, DeliveryAreaSet deliveryAreaSet, Boolean runtimeComparison) {
		...
		//Example: Get start and multiplier of travel time
		//TravelTime time1= (TravelTime) travelTimeSet.getElements().get(0);
		//Double start1 = time1.getStart();
		//Double multiplier1=ProbabilityDistributionService.getMeanByProbabilityDistribution(time1.getProbabilityDistribution())
		
		this.deliveryAreaSet = deliveryAreaSet;
		if(runtimeComparison){
			double[][] timeDependentWeighting2 = { { 0, 1, 1, 1, 1,1, 1 },
					{ 0, 1, 1, 1, 1,1, 1 }, { 0, 1, 1, 1, 1,1, 1 },
					{ 0,1, 1, 1, 1,1, 1 } };
			timeDependentWeighting=timeDependentWeighting2;
			System.out.println("Runtime comparison modus");
		}
	}

	public void start() {

		System.out.println("First travel time:"+timeDependentWeighting[0][1]);
...
//			System.out.println(r.getDeliveryAreaId()+" "+r.getAlternativePreferenceList().get(1).getId());
//		}
		...
		int requestBeginGroup = 0;
		while (requestBeginGroup < requests.size()) {

			// Find all requests with same value
			ForecastedOrderRequest request = requests.get(requestBeginGroup++);

			double currentValue = request.getEstimatedValue();
			System.out.println("I start a new group. Value is " + currentValue);
			ArrayList<ForecastedOrderRequest> currentRequestGroup = new ArrayList<ForecastedOrderRequest>();
			currentRequestGroup.add(request);

			for (int i = requestBeginGroup; i < requests.size(); i++) {
				if (requests.get(i).getEstimatedValue().equals(currentValue)) {
					currentRequestGroup.add(requests.get(i));
				//	System.out.println("Da:" +requests.get(i).getDeliveryAreaId());
					requestBeginGroup++;
				} else {
					// Next group begins here
					// Stop loop if value is lower. Possible because sorted
					break;
				}
			}
			System.out.println("Requests size is " + requests.size());
			System.out.println("Group size is " + currentRequestGroup.size());
			// Determine lowest possible insertion cost per request of the group
			while (currentRequestGroup.size() > 0) { // Recalculate after each
														// insertion because can
														// change after one was
														// inserted
				
				ArrayList<ForecastedOrderRequest> bufferRequests = new ArrayList<ForecastedOrderRequest>();
				for (int requestID = 0; requestID < currentRequestGroup.size(); requestID++) {

					ForecastedOrderRequest currentRequest = currentRequestGroup.get(requestID);
					// Determine preferred window
					// TODO: At the moment, only works with one possible
					// alternative
					// that refers to exactly one time window
					TimeWindow preferredTimeWindow =  currentRequest.getAlternativePreferenceList().get(1)
							.getTimeWindows().get(0);
					...
					ArrayList<InsertionCosts> insertionCosts = this.computeAndSortInsertionCosts(preferredTimeWindowId,
							vehicleRoutes, timeWindows, currentRequest, before, after, this.distances);

					// Insert into routing
					Double cheapestInsertionCosts = this.tryCheapestInsertionPrecalculate(vehicleRoutes,
							preferredTimeWindowId, timeWindows, insertionCosts, currentRequest, before, after);

					if (insertionSuccessful == true) {
						
						currentRequest.setLowestPossibleInsertionCosts(cheapestInsertionCosts);
						bufferRequests.add(currentRequest);
						if(currentRequest.getLowestPossibleInsertionCosts()==null) System.out.println("I added null");
					} else {// Insertion not possible. Do not consider anymore.
				...
				currentRequestGroup= new ArrayList<ForecastedOrderRequest>();
				for(int i = 0; i < bufferRequests.size(); i++)
					currentRequestGroup.add(bufferRequests.get(i));
				
				if (currentRequestGroup.size() > 0) {
					System.out.println("size:"+currentRequestGroup.size());
//					for(int i=0; i < currentRequestGroup.size(); i++){
//						System.out.println(currentRequestGroup.get(i).getLowestPossibleInsertionCosts());
//					}
					Collections.sort(currentRequestGroup, new ForecastedOrderRequestInsertionCostAscComparator());
					ForecastedOrderRequest requestToInsert = currentRequestGroup.get(0);
					System.out.println("I insert Da:" +requestToInsert.getDeliveryAreaId());
					currentRequestGroup.remove(0);

					// System.out.println("Calculated costs of group. size is
					// now "+currentRequestGroup.size());
					// Do insertion as before
					TimeWindow preferredTimeWindow = requestToInsert.getAlternativePreferenceList().get(1)
							.getTimeWindows().get(0);
					int preferredTimeWindowId = 0;
					// if(timeWindows.size()==3){ //TODO: adjust!! Only now
					// hard-coded for travel times with 3 time windows
					// preferredTimeWindowId=1;
					// }
					for (int twID = 0; twID < timeWindows.size(); twID++) {
						if (preferredTimeWindow.getId() == timeWindows.get(twID).getId()) {
							preferredTimeWindowId = preferredTimeWindowId + twID;
					...
					if (test == true) {
						System.out.println("PreferredTimeWindow: "
								+ requestToInsert.getAlternativePreferenceList().get(preferredTimeWindow.getId()));
					}
	...
	private ArrayList<InsertionCosts> computeAndSortInsertionCosts(int preferredTimeWindow,
		...
		for (int r = 0; r < routes.size(); r++) {
			...
			if (routes.get(r).get(preferredTimeWindow).isEmpty()) {
				computeInsertionCostsEmpty(r, routes, preferredTimeWindow, forecastedOrderRequest, distances,
				...
				for (int c = 0; c < routes.get(r).get(preferredTimeWindow).size(); c++) {
					...
					if (routes.get(r).get(preferredTimeWindow).get(c).getId() == routes.get(r).get(preferredTimeWindow)
							.get(0).getId()) {
						computeInsertionCostsInsertionBegin(r, routes, preferredTimeWindow, forecastedOrderRequest,
								distances, insertionCosts, before, after);
					}
					...
					if (routes.get(r).get(preferredTimeWindow).get(c).getId() == routes.get(r).get(preferredTimeWindow)
						...
						computeInsertionCostsInsertionEnd(r, routes, preferredTimeWindow, forecastedOrderRequest,
								distances, insertionCosts, before, after);

						// insertion costs: customer is inserted within (between
						// customers) of preferred time window
					} else {
						computeInsertionCostsInsertionWithin(r, routes, preferredTimeWindow, forecastedOrderRequest,
								distances, insertionCosts, c, before, after);
					}
				}
			}
		}
	...
	private void computeInsertionCostsEmpty(int r, ArrayList<ArrayList<ArrayList<RouteElement>>> routes,
		...
		directionXj = LocationService.getDirection(forecastedOrderRequest.getClosestNode(), routes.get(r)
				.get(preferredTimeWindow + after.get(r).getAfter()).get(0).getForecastedOrderRequest().getClosestNode(),
	...
	private void computeInsertionCostsInsertionWithin(int r, ArrayList<ArrayList<ArrayList<RouteElement>>> routes,
		...
		if (c < routes.get(r).get(preferredTimeWindow).size() - 1) {
			...
			distanceIx = LocationService.getDistanceBetweenAreas(distances,
					...
					forecastedOrderRequest.getClosestNode(), deliveryAreaSet, distanceAreas);
			distanceXj = LocationService.getDistanceBetweenAreas(distances, forecastedOrderRequest.getClosestNode(),
					routes.get(r).get(preferredTimeWindow).get(c + 1).getForecastedOrderRequest().getClosestNode(),
					deliveryAreaSet, distanceAreas);
			distanceIj = LocationService.getDistanceBetweenAreas(distances,
					routes.get(r).get(preferredTimeWindow).get(c).getForecastedOrderRequest().getClosestNode(),
					routes.get(r).get(preferredTimeWindow).get(c + 1).getForecastedOrderRequest().getClosestNode(),
	...
	private void computeInsertionCostsInsertionBegin(int r, ArrayList<ArrayList<ArrayList<RouteElement>>> routes,
		...
		distanceXj = LocationService.getDistanceBetweenAreas(distances, forecastedOrderRequest.getClosestNode(),
				routes.get(r).get(preferredTimeWindow).get(0).getForecastedOrderRequest().getClosestNode(),
				deliveryAreaSet, distanceAreas);
		distanceIj = LocationService.getDistanceBetweenAreas(distances,
				routes.get(r).get(preferredTimeWindow - before.get(r).getBefore())
						.get(routes.get(r).get(preferredTimeWindow - before.get(r).getBefore()).size() - 1)
	...
	private Double tryCheapestInsertionPrecalculate(ArrayList<ArrayList<ArrayList<RouteElement>>> routes,
		...
		while (insertionSuccessful == false) {
			...
			if (insertionCosts.size() > 0) {
				...
				//if(currentlyBest.getInsertionCosts()==null) System.out.println("currently best is null");
...
if(currentlyBest==null) System.out.println("currently best is null");
	...
	private void tryInsertionPrecalculate(int r, ArrayList<ArrayList<ArrayList<RouteElement>>> routes,
			...
			ArrayList<InsertionCosts> insertionCosts, ForecastedOrderRequest forecastedOrderRequest, int before,
			int after) {
									...
									if (insertionCosts_Waiting > 0) {
										...
										if (routes.get(r).get(preferredTimeWindow)
												...
												+ insertionCosts.get(0).getDistanceIx() <= timeWindow
														.get(preferredTimeWindow - 1).getEndTime() * 60
					...
					if (insertionSuccessful == false) {
						...
						if (routes.get(r).get(preferredTimeWindow - before)
							...
							insertionCosts_Waiting = routes.get(r).get(preferredTimeWindow - before)
									...
									- timeWindow.get(preferredTimeWindow - 1).getStartTime() * 60
	...
	public void insertCustomerEmptyTimeWindow(int r, ArrayList<ArrayList<ArrayList<RouteElement>>> routes,
		...
		element.setSlack(
				Math.max(0, timeWindows.get(preferredTimeWindow - 1).getEndTime() * 60 - element.getServiceBegin()));
	...
	private void insertCustomerWithinTimeWindow(int r, ArrayList<ArrayList<ArrayList<RouteElement>>> routes,
		...
		if (check == 0) {
			...
			for (int j = preferredTimeWindow; j < routes.get(r).size(); j++) {
				...
				for (int i = setCheckFromHereForLoop; i < routes.get(r).get(j).size(); i++) {
					...
					routes.get(r).get(j).get(i)
							.setSlack(Math.max(0, routes.get(r).get(j).get(i).getTimeWindow().getEndTime() * 60
			...
			element.setSlack(Math.max(0,
					timeWindows.get(preferredTimeWindow - 1).getEndTime() * 60 - element.getServiceBegin()));
			...
			for (int j = preferredTimeWindow; j < routes.get(r).size(); j++) {
				...
				for (int i = setCheckFromHereForLoop; i < routes.get(r).get(j).size(); i++) {
					...
					routes.get(r).get(j).get(i).setServiceBegin(
							 (routes.get(r).get(j).get(i).getServiceBegin() + insertionCosts_Waiting));
					routes.get(r).get(j).get(i)
							.setSlack(Math.max(0f, routes.get(r).get(j).get(i).getTimeWindow().getEndTime() * 60
									- routes.get(r).get(j).get(i).getServiceBegin()));
