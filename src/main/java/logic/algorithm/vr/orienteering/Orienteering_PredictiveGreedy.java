...
/**
 * Orienteering_PredictiveGreedy
 */

public class Orienteering_PredictiveGreedy   implements RoutingAlgorithm{
	...
	public Orienteering_PredictiveGreedy(ArrayList<ForecastedOrderRequest> requests, TimeWindowSet timeWindowSet, Depot depot, ArrayList<Vehicle> vehicles, ArrayList<NodeDistance> distances, ArrayList<Node> nodes, TravelTimeSet travelTimeSet, DeliveryAreaSet deliveryAreaSet) {
	...
	public void start (){
		...
		ForecastedOrderRequest i1;
		ForecastedOrderRequest i2;

		for (int n = requests.size(); n > 1; n = n - 1) {
			for (int i = 0; i < requests.size() - 1; i++) {
				if (requests.get(i).getEstimatedValue() < requests.get(i + 1).getEstimatedValue()) {
					i1 = requests.get(i);
					i2 = requests.get(i + 1);
					requests.set(i, i2);
					requests.set((i + 1), i1);
