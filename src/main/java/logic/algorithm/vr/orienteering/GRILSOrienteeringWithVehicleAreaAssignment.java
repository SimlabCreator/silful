...
public class GRILSOrienteeringWithVehicleAreaAssignment implements RoutingAlgorithm {
	...
	private static double minProb = 0.8;
	...
	private boolean weightWithTWProb;
	private boolean weightWithArrivalProb;
	private boolean considerImportanceForInsertion;
	...
	private static String[] paras = new String[] { "greediness_upperBound", "greediness_lowerBound",
			...
			"includeDriveFromStartingPosition", "duplicate_segments", "weight_time_window_prob",
			"weight_arrival_prob", "imp_for_insertion" };

	public GRILSOrienteeringWithVehicleAreaAssignment(Region region, OrderRequestSet orderRequestSet,
			...
			Double directDistances, Double includeDriveFromStartingPosition, int orderHorizonLength,
			Double arrivalProbability, Double duplicateSegments, Double weightWithTWProb,
			Double weightWithArrivalProb, Double considerImportanceForInsertion) {
		this.region = region;
		...
		this.weightWithTWProb = (weightWithTWProb == 1.0);
		this.weightWithArrivalProb = (weightWithArrivalProb == 1.0);
		...
		this.expectedNumberOfArrivals = arrivalProbability * orderHorizonLength;
		this.duplicateSegments = (duplicateSegments == 1);
		this.demandSegmentWeightingOriginal = demandSegmentWeighting;
		this.considerImportanceForInsertion=(considerImportanceForInsertion==1.0);
	}
	...
	public void start() {
		...
		for (Integer routeId : bestSolution.keySet()) {
			...
			RouteElement elementBefore =null;
			for (RouteElement e : bestSolution.get(routeId)) {
				...
				if(e.getTravelTime()!=LocationService.calculateHaversineDistanceBetweenCustomers(e.getOrder().getOrderRequest().getCustomer(),
						elementBefore.getOrder().getOrderRequest().getCustomer())* distanceMultiplierAsTheCrowFlies / region.getAverageKmPerHour() * TIME_MULTIPLIER) {
					System.out.println("Something wrong with distances. difference: "+(e.getTravelTime()-LocationService.calculateHaversineDistanceBetweenCustomers(e.getOrder().getOrderRequest().getCustomer(),
							elementBefore.getOrder().getOrderRequest().getCustomer())* distanceMultiplierAsTheCrowFlies / region.getAverageKmPerHour() * TIME_MULTIPLIER));
				}
				}
	...
	private void fillUpRoutes(Double greedinessFactor, boolean again, int fillUpCounter,
		...
		while (this.orderRequests.size() > 0) {
			...
			if(considerImportanceForInsertion){
			...
			for (RouteElement e : insertionOptions) {
				...
				if(this.considerImportanceForInsertion){
					e.setTempValue(
	...
	private HashMap<Integer, RouteElement> findCheapestInsertionOption(OrderRequest request, int fillUpCounter,
		...
		for (Integer twId : bestElementPerTw.keySet()) {
			...
			if(this.weightWithArrivalProb){
				
				double probArrivals = 1-ProbabilityDistributionService
						.getBinominalCummulativeProbabilityValue(this.orderHorizonLength,
								this.arrivalProbability * this.daLowerWeights.get(request.getCustomer().getTempDeliveryArea()), this.bufferAlreadyAcceptedPerDeliveryArea
								.get(request.getCustomer().getTempDeliveryArea())+1)+ProbabilityDistributionService.getBinominalProbabilityValue(this.orderHorizonLength,
								this.arrivalProbability * this.daLowerWeights.get(request.getCustomer().getTempDeliveryArea()), this.bufferAlreadyAcceptedPerDeliveryArea
								.get(request.getCustomer().getTempDeliveryArea())+1);
				if(probArrivals<= minProb){
					
					value=value*probArrivals;
				}
			}
					...
					if (((tw.getEndTime() - this.timeWindowSet.getTempStartOfDeliveryPeriod())
						...
						if (maximumPushOfNext >= shift) { // Feasible with
							...
							if (useTimeWindow) {
								...
								if(travelTimeTo!=LocationService.calculateHaversineDistanceBetweenCustomers(request.getCustomer(),
										this.routes.get(routeId).get(position-1).getOrder().getOrderRequest().getCustomer())* distanceMultiplierAsTheCrowFlies / region.getAverageKmPerHour() * TIME_MULTIPLIER) {
									System.out.println("Something wrong with distances. difference: "+(travelTimeTo-LocationService.calculateHaversineDistanceBetweenCustomers(request.getCustomer(),
										this.routes.get(routeId).get(position-1).getOrder().getOrderRequest().getCustomer())* distanceMultiplierAsTheCrowFlies / region.getAverageKmPerHour() * TIME_MULTIPLIER));
								}
	...
	public void insertRouteElement(RouteElement element) {
		...
		if(element.getTravelTimeFrom()!=LocationService.calculateHaversineDistanceBetweenCustomers(element.getOrder().getOrderRequest().getCustomer(),
				route.get(element.getTempPosition() + 1).getOrder().getOrderRequest().getCustomer())* distanceMultiplierAsTheCrowFlies / region.getAverageKmPerHour() * TIME_MULTIPLIER) {
			System.out.println("Something wrong with distances. difference: "+(element.getTravelTimeFrom()-LocationService.calculateHaversineDistanceBetweenCustomers(element.getOrder().getOrderRequest().getCustomer(),
					route.get(element.getTempPosition() + 1).getOrder().getOrderRequest().getCustomer())* distanceMultiplierAsTheCrowFlies / region.getAverageKmPerHour() * TIME_MULTIPLIER));
		}
	...
	private void removeRouteElementsOfRoute(ArrayList<RouteElement> route, int routeId, int numberOfRemovalsPerRoute,
		...
		route.get(positionToRemove).setTravelTimeTo(travelTimeTo);
	...
	private Double evaluateSolution(HashMap<Integer, ArrayList<RouteElement>> solution) {
		...
		HashMap<DeliveryArea, ArrayList<Double>> collectedPerArea = new HashMap<DeliveryArea, ArrayList<Double>>();
		...
		for (Integer routeId : solution.keySet()) {
			...
			for (int elementId = 1; elementId < elements.size() - 1; elementId++) {
				...
				if (this.weightWithArrivalProb) {
					...
					if (!collectedPerArea.containsKey(
							elements.get(elementId).getOrder().getOrderRequest().getCustomer().getTempDeliveryArea())) {
						collectedPerArea.put(elements.get(elementId).getOrder().getOrderRequest().getCustomer()
								.getTempDeliveryArea(), new ArrayList<Double>());
					}
					collectedPerArea.get(
							elements.get(elementId).getOrder().getOrderRequest().getCustomer().getTempDeliveryArea())
							.add(elementValue);

				} else {
					collectedValue += elementValue;
				}
			}
		}
		...
		if (this.weightWithArrivalProb) {
			...
			for (DeliveryArea area : collectedPerArea.keySet()) {
				Collections.sort(collectedPerArea.get(area), Collections.reverseOrder());
				for (int i = 0; i < collectedPerArea.get(area).size(); i++) {
					double probArrivals =1-ProbabilityDistributionService
					.getBinominalCummulativeProbabilityValue(this.orderHorizonLength,
							this.arrivalProbability * this.daLowerWeights.get(area), i+1)+ProbabilityDistributionService.getBinominalProbabilityValue(this.orderHorizonLength,
							this.arrivalProbability * this.daLowerWeights.get(area), i+1);
					if (probArrivals>minProb) {
						collectedValue += collectedPerArea.get(area).get(i);
					} else {
						
						
						collectedValue += collectedPerArea.get(area).get(i) * probArrivals;
					}
				}
			}
		}
