 ...
 * Sorts all orders based on insertion costs and score
 * inserts customer with short-term best ratio of score and costs
 * 
 * @author C. K�hler
 *
 */

public class Orienteering_IteratedLocalSearch   implements RoutingAlgorithm{
	...
	private double bestSolutionScore;
	...
	public Orienteering_IteratedLocalSearch(ArrayList<ForecastedOrderRequest> requests, TimeWindowSet timeWindowSet, Depot depot, ArrayList<Vehicle> vehicles, ArrayList<NodeDistance> distances, ArrayList<Node> nodes, TravelTimeSet travelTimeSet, DeliveryAreaSet deliveryAreaSet) {
	...
	public void start (){
		...
		//iterated local search variables
		...
		//TODO
		while(numberOfTimesNoImprovement < 150){
			
			//System.out.println("SIZE NOT: " + notAcceptedRequests.size());
			...
			if (solutionScore > bestSolutionScore) {
				...
				int minRouteSize = vehicleRoutes.get(0).size();
				int numberCustomers = 0;
				
				for(int route = 0; route < vehicleRoutes.size(); route++){
					
					if(vehicleRoutes.get(route).size() < minRouteSize){
						minRouteSize = vehicleRoutes.get(route).size();
					}
					
					numberCustomers = numberCustomers + vehicleRoutes.get(route).size();
					
				}

				
				minRouteSize = minRouteSize -2;
				
							
				if(s >= minRouteSize){
					s = Math.max(1, s - minRouteSize);
				}
				...
				doShakeStep(s, r, vehicleRoutes, notAcceptedRequests);
...
//
		...
		// Transfer routing into final format
				...
				this.routing.setAdditionalInformation(""+this.bestSolutionScore);
	...
	public void start(ArrayList<ArrayList<RouteElement>> vehicleRoutes, ArrayList<ForecastedOrderRequest> notAcceptedRequests, ArrayList<NodeDistance> distances) {
		...
		//System.out.println("VOR START: " + vehicleRoutes.get(0).size() + " " + vehicleRoutes.get(1).size() + " " + vehicleRoutes.get(2).size());
		...
		// TODO: delete later, only hard - coded if 3 time windows for travel times!!!
		if (timeWindows.size() == 3) {timeWindowCount3 = true;} else {timeWindowCount3 = false;}
		...
		for (int requestIdOuter=0; requestIdOuter < requests.size(); requestIdOuter++) {
			...
			//compares score of all requests	
//			
//			System.out.println(requests.size()-25);
//			System.out.println("Size INSERTIONCRITERION: " + insertionCriterion.size());
			
			compareScore(insertionCriterion);
				
		//	}} //WENN HIER ZWEI KLAMMERN DANN AUCH DIE GUTEN KUNDEN
			...
			if (insertionCriterion.size() > 0) {
				//insert request with at that moment best score into route		
				insertRequestIntoRoute(vehicleRoutes, possibleRequestsForInsertion.get(insertionCriterion.get(insertionCriterion.size()-1).getPosition()), timeWindows);
		...
		}//2/2 WENN SO DIE KLAMMERN DANN BESSER SHAKE
			
		
		for(int z = 0; z < requests.size(); z++){if(requests.get(z).isAccepted() == false){notAcceptedRequests.add(requests.get(z));}}	

		//System.out.println("NACH START: " + vehicleRoutes.get(0).size() + " " + vehicleRoutes.get(1).size() + " " + vehicleRoutes.get(2).size());
...
//		System.out.println("HIER: " + request.getEstimatedValue());
	...
	public void doShakeStep(int s, int r, ArrayList<ArrayList<RouteElement>> routes, ArrayList<ForecastedOrderRequest> notAcceptedRequests) {


	//	System.out.println("s: " + s);
	//	System.out.println("r: " + r);
	//	System.out.println("VOR SHAKE: " + routes.get(0).size() + " " + routes.get(1).size() + " " + routes.get(2).size());
		...
		for (int route = 0; route < routes.size(); route++) {
			...
			if (routes.get(route).size() <= (s + r)) {
				
				r = routes.get(route).size() - s - 1;
				...
				for (int i = 0; i < r; i++) {
					...
					ForecastedOrderRequest f = new ForecastedOrderRequest();
					
					f.setAccepted(false);
					f.setForecastedOrderRequest(routes.get(route).get(s).getForecastedOrderRequest());
					f.setEstimatedValue(routes.get(route).get(s).getEstimatedValue());
					f.setServiceTime(routes.get(route).get(s).getServiceTime());
					f.setId(routes.get(route).get(s).getId());
					f.setDeliveryArea(routes.get(route).get(s).getDeliveryArea());
					f.setDeliveryAreaId(routes.get(route).get(s).getDeliveryAreaId());
					f.setClosestNode(routes.get(route).get(s).getForecastedOrderRequest().getClosestNode());
					f.setClosestNodeId(routes.get(route).get(s).getForecastedOrderRequest().getClosestNodeId());
					f.setAlternativePreferenceList(routes.get(route).get(s).getForecastedOrderRequest().getAlternativePreferenceList());
					f.setTimeWindow(routes.get(route).get(s).getTimeWindow());
					f.setTimeWindowId(routes.get(route).get(s).getTimeWindowId());
					f.setEstimatedValue(routes.get(route).get(s).getEstimatedValue());
					
					//f.setServiceBegin(routes.get(route).get(s).getServiceBegin());
					//f.setServiceTime(routes.get(route).get(s).getServiceTime());
					//f.setTravelTimeFrom(routes.get(route).get(s).getTravelTimeFrom());
					//f.setTravelTimeTo(routes.get(route).get(s).getTravelTimeTo());
					//f.setSlack(routes.get(route).get(s).getSlack());
					//f.setWaitingTime(routes.get(route).get(s).getWaitingTime());
					
					
					notAcceptedRequests.add(f);
					
					// never remove depot
					if (s != 0 || s != routes.get(route).size()) {
						
						routes.get(route).remove(s);
					//}
				}
			}	
		}	
	...
	//	System.out.println("NACH SHAKE: " + routes.get(0).size() + " " + routes.get(1).size() + " " + routes.get(2).size());
		updateRouteAfterShakeStep(routes, r, s, distances);


	}
	
	public ArrayList<ArrayList<RouteElement>> updateRouteAfterShakeStep (ArrayList<ArrayList<RouteElement>> vehicleRoutes, int r, int s, ArrayList<NodeDistance> distances){
		...
		for(int route=0; route<vehicleRoutes.size(); route++){
			...
			if (vehicleRoutes.get(route).size() <= s) {
				route++;
			} else {
			
			for(int c=s; c<= vehicleRoutes.get(route).size()-1; c++){
				
				
				
				double distanceIj; // Ij: travelTime to
				double distanceJi; // Xj: travelTime from 

				
				int directionIj ; // direction to 
				int directionJi; //  direction from


				// get distances between two nodes
				 distanceIj = LocationService.getDistanceBetweenAreas(
						 distances, 
						 vehicleRoutes.get(route).get(c - 1).getForecastedOrderRequest().getClosestNode(), 
						 vehicleRoutes.get(route).get(c).getForecastedOrderRequest().getClosestNode(), 
						 deliveryAreaSet, 
						 distanceAreas);
				 distanceJi = LocationService.getDistanceBetweenAreas(
						 distances,  
						 vehicleRoutes.get(route).get(c - 1).getForecastedOrderRequest().getClosestNode(),
						 vehicleRoutes.get(route).get(c).getForecastedOrderRequest().getClosestNode(),						 
						 deliveryAreaSet, 
				 ...
				 directionJi = LocationService.getDirection(
						 vehicleRoutes.get(route).get(c - 1).getForecastedOrderRequest().getClosestNode(),
						 vehicleRoutes.get(route).get(c).getForecastedOrderRequest().getClosestNode(),
						 deliveryAreaSet, 
						 centerArea);
				...
				if(c==0){
					currentTimeWindowId=1;
				}else if(c== vehicleRoutes.get(route).size()-1){
					currentTimeWindowId= numberOfTimeWindows;
				}else{
					if(vehicleRoutes.get(route).get(c).getServiceBegin() < 720){currentTimeWindowId = 1;}
					if(vehicleRoutes.get(route).get(c).getServiceBegin() >= 720 && vehicleRoutes.get(route).get(c).getServiceBegin() < 840){currentTimeWindowId = 2;}
					if(vehicleRoutes.get(route).get(c).getServiceBegin() >= 840 && vehicleRoutes.get(route).get(c).getServiceBegin() < 2000){currentTimeWindowId = 3;}
				}
				...
				distanceIj = distanceIj/timeDependentWeighting[directionIj][relevantWindow];
				distanceJi = distanceJi/timeDependentWeighting[directionJi][relevantWindow];
						
				vehicleRoutes.get(route).get(c - 1).setTravelTimeFrom(distanceIj);
				vehicleRoutes.get(route).get(c).setTravelTimeTo(newDistance = distanceJi);
				vehicleRoutes.get(route).get(c).setServiceBegin(Math.max(vehicleRoutes.get(route).get(c).getStartTimeWindow(), vehicleRoutes.get(route).get(c - 1).getServiceBegin() + vehicleRoutes.get(route).get(c - 1).getServiceTime() + newDistance));
				vehicleRoutes.get(route).get(c).setSlack(Math.max(0, vehicleRoutes.get(route).get(c).getEndTimeWindow() - vehicleRoutes.get(route).get(c).getServiceBegin()));
			    vehicleRoutes.get(route).get(c).setWaitingTime(Math.max(0, (vehicleRoutes.get(route).get(c).getStartTimeWindow() - (vehicleRoutes.get(route).get(c - 1).getServiceBegin() + vehicleRoutes.get(route).get(c - 1).getServiceTime())- vehicleRoutes.get(route).get(c - 1).getTravelTimeFrom())));
				}
			}
		}
